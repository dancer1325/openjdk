...
|  | JEP 530: Primitive Types in Patterns, instanceof, and switch (Fourth Preview)](530) | 
| Reviewed by | Alex Buckley, Brian Goetz | 
| Endorsed by | Brian Goetz | 
| Created | 2025/02/03 12:20 | 
| Updated | 2025/10/02 17:52 | 
| Issue | 8349215](https://bugs.openjdk.org/browse/JDK-8349215) | 
...
This feature was originally proposed byJEP 455](https://openjdk.org/jeps/455)(JDK 23) and re-previewed byJEP 488](https://openjdk.org/jeps/488)(JDK 24), without change. We here propose to preview it for a third time, without change.
...
In Java 21, primitive type patterns are permitted only as nested patterns in record patterns, and only when they name the type of the match candidate exactly, as in:
...
- 
Further enhance the`switch`construct so that it works with all primitive types, not just a subset of theintegral primitive types](https://docs.oracle.com/javase/specs/jls/se24/html/jls-4.html#jls-4.2).
...
Comparing this table to its equivalent inJLS §5.5](https://docs.oracle.com/javase/specs/jls/se24/html/jls-5.html#jls-5.5-320), it can be seen that many of the conversions permitted by`ω`in JLS §5.5 are "upgraded" to the unconditionally exact`ɛ`above.
...
To enable primitive types in the`instanceof`type test operator, we remove the restrictions (JLS §15.20.2](https://docs.oracle.com/javase/specs/jls/se24/html/jls-15.html#jls-15.20.2)) that the type of the left-hand operand must be a reference type and that the right-hand operand must specify a reference type. The type test operator becomes
...
One pattern*dominates*another pattern if it matches all the values that the other pattern matches. For example, the type pattern`Object o`dominates the type pattern`String s`because everything that would match`String s`would also match`Object o`. In a`switch`, it is illegal for a`case`label with an unguarded type pattern`P`to precede a case label with type pattern`Q`if`P`dominates`Q`. The meaning of dominance is unchanged, but now applies to primitive types as well as reference types: A type pattern`T t`dominates a type pattern`U u`if`T t`would be unconditional on a match candidate of type`U`.
