...
| Effort | XL | 
| Duration | XL | 
| Relates to | JEP 244: TLS Application-Layer Protocol Negotiation Extension](244) | 
|  | JEP 321: HTTP Client API](321) | 
| Reviewed by | Alan Bateman | 
| Endorsed by | Brian Goetz | 
| Created | 2014/05/12 16:26 | 
| Updated | 2022/05/31 16:17 | 
| Issue | 8042950](https://bugs.openjdk.org/browse/JDK-8042950) | 

## Summary

Define a new HTTP client API that implements HTTP/2 and WebSocket, and can replace the legacy`HttpURLConnection`API. The API will be delivered as an incubator module, as defined inJEP 11](http://openjdk.java.net/jeps/11), with JDK 9. This implies:
...
- 
Must supportHTTP/2](http://http2.github.io/). (The application-level semantics of HTTP/2 are mostly the same as 1.1, though the wire protocol is completely different.)
...
A number of existing HTTP client APIs and implementations exist, e.g.,Jetty](http://eclipse.org/jetty)and theApache HttpClient](https://hc.apache.org/httpcomponents-client-ga/). Both of these are both rather heavy-weight in terms of the numbers of packages and classes, and they don't take advantage of newer language features such as lambda expressions.
