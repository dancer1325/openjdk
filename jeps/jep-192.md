...
| Issue | 8046182](https://bugs.openjdk.org/browse/JDK-8046182) | 
...
```
private final char&lbrack;&rbrack; value
private int hash
```
...
- 
Both the deduplication queue and hashtable stores`oop`s which are treated as weak references from a GC point of view. This means that the GC needs to traverse both structures to adjust or remove references to objects that were moved or garbage collected. Traversing the queue and the hashtable is the most performance-critical part of this feature. The traversal is done in parallel using all GC worker threads.
