...
| Updated | 2024/08/02 14:44 | 
| Issue | 8229469](https://bugs.openjdk.org/browse/JDK-8229469) | 
...
Musl](https://musl.libc.org)is an implementation, for Linux-based systems, of the standard library functionality described in the ISO C and POSIX standards. Several Linux distributions includingAlpine Linux](https://alpinelinux.org/)andOpenWrt](https://openwrt.org/)are based on musl, while some others provide an optional musl package (e.g.,Arch Linux](https://www.archlinux.org/)).

The Alpine Linux distribution is widely adopted in cloud deployments, microservices, and container environments due to its small image size. A Dockerbase image for Alpine Linux](https://hub.docker.com/_/alpine), for example, is less than 6 MB. Enabling Java to run out-of-the-box in such settings will allow Tomcat, Jetty, Spring, and other popular frameworks to work in such environments natively.

By using`jlink`(JEP 282)](https://openjdk.java.net/jeps/282)to reduce the size of the Java runtime, a user will be able to create an even smaller image targeted to run a specific application. The set of modules required by an application can be determined via the`jdeps`command. For example, if a target application depends only on the`java.base`module then a Docker image with Alpine Linux and a Java runtime with just that module and the server VM fits in 38 MB.

The same motivation applies to embedded deployments, which also have size constraints.

## Description

This JEP intends to integrate thePortola Project](https://openjdk.java.net/projects/portola/)upstream.
