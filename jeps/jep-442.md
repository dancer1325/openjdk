...
| Relates to | JEP 434: Foreign Function & Memory API (Second Preview)](434) | 
|  | JEP 454: Foreign Function & Memory API](454) | 
| Reviewed by | Alex Buckley, Jorn Vernee | 
| Endorsed by | Mark Reinhold | 
| Created | 2023/02/01 14:58 | 
| Updated | 2023/09/27 16:27 | 
| Issue | 8301625](https://bugs.openjdk.org/browse/JDK-8301625) | 
...
The Foreign Function & Memory (FFM) API first previewed in JDK 19 viaJEP 424](https://openjdk.org/jeps/424), then previewed again in JDK 20 viaJEP 434](https://openjdk.org/jeps/434). This JEP proposes a third preview to incorporate refinements based on feedback. In this version we have:

- Centralized the management of the lifetimes of native segments in the`Arena`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/Arena.html)interface;
...
- Provided a fallback native linker implementation, based on`libffi`](https://github.com/libffi/libffi), to facilitate porting; and
...
(`MemorySegment`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/MemorySegment.html),`Arena`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/Arena.html), and`SegmentAllocator`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/SegmentAllocator.html)),
- Manipulate and access structured foreign memory
(`MemoryLayout`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/MemoryLayout.html)and`VarHandle`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/VarHandle.html)), and
- Call foreign functions (`Linker`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/Linker.html),`FunctionDescriptor`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/FunctionDescriptor.html), and`SymbolLookup`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/SymbolLookup.html)).

The FFM API resides in the`java.lang.foreign`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/package-summary.html)package of the`java.base`module.
...
Because the FFM API is apreview API](https://openjdk.org/jeps/12), you must compile and run the code with preview features enabled, i.e.,`javac --release 21 --enable-preview ...`and`java --enable-preview ...`.
...
A*memory segment*](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/MemorySegment.html)is an abstraction backed by a contiguous region of memory, located either off-heap or on-heap. A memory segment can be
...
The temporal bounds of a segment are determined by the*arena*](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/Arena.html)used to allocate the segment. Multiple segments allocated in the same arena have the same temporal bounds, and can safely contain mutual references: Segment`A`can hold a pointer to an address in segment`B`, and segment`B`can hold a pointer to an address in segment`A`, and both segments will be deallocated at the same time so that neither segment has a dangling pointer.
...
All these characteristics are captured in the`ValueLayout`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/ValueLayout.html)abstraction. For example, the predefined`JAVA_INT`value layout is four bytes wide, is aligned on four-byte boundaries, uses the native platform endianness (e.g., little-endian on Linux/x64), and is associated with the Java type`int`.
...
To reduce the need for tedious calculations about memory layout (e.g.,`(i * 2) + 1`in the example above), a`MemoryLayout`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/MemoryLayout.html)can be used to describe the content of a memory segment in a more declarative fashion. For example, the desired layout of the native memory segment in the examples above can be described via:
...
This creates a*sequence memory layout*containing ten repetitions of a*struct layout*whose elements are two`JAVA_INT`layouts named`x`and`y`, respectively. Given this layout, we can avoid calculating offsets in our code by creating two*memory-access var handles*, specialvar handles](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/VarHandle.html)which accept a`MemorySegment`parameter (the segment to be dereferenced) followed by one or more`long`coordinates (the indices at which the dereference operation should occur):
...
Memory allocation is often a bottleneck when clients use off-heap memory. The FFM API therefore includes a`SegmentAllocator`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/SegmentAllocator.html)abstraction to define operations to allocate and initialize memory segments. As a convenience, the`Arena`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/Arena.html)class implements the`SegmentAllocator`interface so that arenas can be used to allocate native segments. In other words,`Arena`is a "one stop shop" for flexible allocation and timely deallocation of off-heap memory:
...
Segment allocators can also be obtained via factories in the`SegmentAllocator`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/SegmentAllocator.html)interface. For example, one factory creates a*slicing allocator*](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/SegmentAllocator.html#slicingAllocator(java.lang.foreign.MemorySegment))that responds to allocation requests by returning memory segments which are part of a previously allocated segment; thus, many requests can be satisfied without physically allocating more memory. The following code obtains a slicing allocator over an existing segment and then uses it to allocate a segment initialized from a Java array:
...
- 
`Linker::defaultLookup()`creates a*default lookup*, which locates all the symbols in libraries that are commonly used on the OS and processor combination associated with the`Linker`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/Linker.html)instance.
...
To create a downcall method handle, the FFM API requires the client to provide a`FunctionDescriptor`that describes the C parameter types and C return type of the target C function. C types are described in the FFM API by`MemoryLayout`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/MemoryLayout.html)objects such as`ValueLayout`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/ValueLayout.html)for scalar C types and`GroupLayout`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/GroupLayout.html)for C struct types. Clients usually have`MemoryLayout`objects on hand to dereference data in foreign memory, and can reuse them to obtain a`FunctionDescriptor`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/FunctionDescriptor.html).
...
A*calling convention*enables interoperation between different languages by specifying how code in one language invokes a function in another language, passes arguments, and receives results. The`Linker`API is neutral with respect to calling conventions, but the native linker implementation supports several calling conventions out-of-the-box: Linux/x64, Linux/AArch64, Linux/RISC-V, macOS/x64, macOS/AArch64, Windows/x64 and Windows/AArch64.Additional platforms](https://github.com/libffi/libffi#supported-platforms)are supported via the*fallback linker*, which is a native linker implementation based on`libffi`](https://github.com/libffi/libffi). Being written in Java, the`Linker`API is far easier to maintain and extend than JNI, whose calling conventions are hardwired into HotSpot's C++ code.
...
Clients can turn a zero-length memory segment into a native segment of their chosen size via the method`MemorySegment::reinterpret`](https://cr.openjdk.org/~pminborg/panama/21/v1/javadoc/java.base/java/lang/foreign/MemorySegment.html#reinterpret(long,java.lang.foreign.Arena,java.util.function.Consumer)). This method attaches fresh spatial and temporal bounds to a zero-length memory segment in order to allow dereference operations. The memory segment returned by this method is unsafe: A zero-length memory segment might be backed by a region of memory that is 10 bytes long, but the client might overestimate the size of the region and use`MemorySegment::reinterpret`to obtain a segment that is 100 bytes long. Later, this might result in attempts to dereference memory outside the bounds of the region, which might cause a JVM crash or — even worse — result in silent memory corruption.

Overriding the spatial and temporal bounds of a zero-length memory segment is unsafe. For this reason, the`MemorySegment::reinterpret`method is*restricted*, and using it in a program causes the Java runtime to issue warnings (see morebelow](#Safety)).
