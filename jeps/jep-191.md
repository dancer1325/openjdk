...
| Effort | M | 
| Duration | L | 
| Created | 2014/01/28 20:00 | 
| Updated | 2025/01/14 21:27 | 
| Issue | 8046181](https://bugs.openjdk.org/browse/JDK-8046181) | 
...
- 
Provide a foreign function interface at the Java level, similar toJNA (Java Native Access)](https://github.com/twall/jna)orJNR (Java Native Runtime)](https://github.com/jnr).
...
For years, Java and JDK developers have had only one trusted way to add native-level features to Java applications: theJava Native Interface (JNI)](https://en.wikipedia.org/wiki/Java_Native_Interface). This interface governs the boundary between the managed environment of the JVM and the unmanaged environment of native code, providing explicit protocols for data marshaling, object lifecycle management, upcalls back into Java, and native JVM tooling and management APIs. JNI provides a strict, unmistakable demarcation between managed and native code; it also makes that boundary incredibly painful to cross, even for the most trivial cases.
...
- 
The JVM security subsystem should understand specific library/fuction coordinates. It should be possible to set up security policies that allow binding only specific functions in specific libraries, rather than just the coarse-grained library-level permissions that exist today.
...
- 
Invoke a function at some offset in that library;
...
The need for a Java FFI has spawned several libraries. Among these,Java Native Access (JNA)](https://github.com/twall/jna)is the most pervasively used, andJava Native Runtime (JNR)](https://github.com/jnr)is perhaps the most comprehensive and advanced. JNR is likely to form the basis for this JEP, since it implements various levels of abstraction, provides function and memory metadata, abstracts away library and function binding, and has been in heavy use by at least theJRuby project](http://jruby.org)(and its users) for at least the last five years.
