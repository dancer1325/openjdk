...
| Effort | M | 
| Duration | M | 
| Created | 2016/12/15 00:01 | 
| Updated | 2025/03/03 18:48 | 
| Issue | 8171275](https://bugs.openjdk.org/browse/JDK-8171275) | 

## Summary

Support Certificate Transparency (CT) in JSSE as specified inRFC 6962](https://tools.ietf.org/html/rfc6962)and section 4.4.2.1 ofRFC 8446](https://tools.ietf.org/html/rfc8446).
...
- from a signed certificate timestamp list extension embedded in the X.509 TLS server certificate.
- from a signed certificate timestamp list extension in a stapled OCSP response. This method of delivery requires that OCSP stapling is active for the TLS handshake (seeJEP 249](https://bugs.openjdk.java.net/browse/JDK-8046321)for more information on OCSP stapling)
- from a TLS extension that is asserted in the ClientHello message and delivered to the client either via the ServerHello (for TLS versions <= TLSv1.2) or in the Certificate message (TLSv1.3).
...
Certificate Transparency Version 2.0](https://datatracker.ietf.org/doc/draft-ietf-trans-rfc6962-bis)is a revised and improved CT solution. This is still in draft but is far along in the process. It is assumed that this will not quite reach standardization by the time this JEP is completed. CT v2.0 design and requirements should be considered when designing classes and interfaces for this JEP.
