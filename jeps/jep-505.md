...
| Relates to | JEP 499: Structured Concurrency (Fourth Preview)](499) | 
|  | JEP 525: Structured Concurrency (Sixth Preview)](525) | 
| Reviewed by | Paul Sandoz | 
| Endorsed by | Paul Sandoz | 
| Created | 2024/09/18 04:58 | 
| Updated | 2025/10/09 07:21 | 
| Issue | 8340343](https://bugs.openjdk.org/browse/JDK-8340343) | 
...
Structured Concurrency incubated inJDK 19](https://openjdk.org/projects/jdk/19/)viaJEP 428](https://openjdk.org/jeps/428)andJDK 20](https://openjdk.org/projects/jdk/20/)viaJEP 437](https://openjdk.org/jeps/437). It previewed inJDK 21](https://openjdk.org/projects/jdk/21/)viaJEP 453](https://openjdk.org/jeps/453), with thefork](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.html#fork(java.util.concurrent.Callable))method changed to return aSubtask](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.Subtask.html)rather than a`Future`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/Future.html). It re-previewed inJDK 22](https://openjdk.org/projects/jdk/22/)viaJEP 462](https://openjdk.org/jeps/462),JDK 23](https://openjdk.org/projects/jdk/23/)viaJEP 480](https://openjdk.org/jeps/480), andJDK 24](https://openjdk.org/projects/jdk/24/)viaJEP 499](https://openjdk.org/jeps/499).

We propose to preview the API once more in JDK 25 with several API changes. In particular, a`StructuredTaskScope`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.html)is now opened via static factory methods rather than public constructors. The zero-parameter`open`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.html#open())factory method covers the common case by creating a`StructuredTaskScope`that waits for all subtasks to succeed or any subtask to fail. Other policies and outcomes can be implemented by providing an appropriate`Joiner`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.Joiner.html)to one of the richer`open`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.html#open(java.util.concurrent.StructuredTaskScope.Joiner))factory methods.
...
The`java.util.concurrent.ExecutorService`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/ExecutorService.html)API, introduced in Java 5, can execute subtasks concurrently.

For example, here is a method,`handle()`, that represents a task in a server application. It handles an incoming request by submitting two subtasks to an`ExecutorService`. One subtask executes the method`findUser()`, while the other executes the method`fetchOrder()`. The`ExecutorService`immediately returns a`Future`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/Future.html)for each subtask, and executes the subtasks concurrently according to its scheduling policy. The`handle()`method awaits the subtasks' results via blocking calls to their futures'`get()`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/Future.html#get())methods, so the task is said to*join*its subtasks.
...
(The Java Platform already has an API for imposing structure on concurrent tasks, namely`java.util.concurrent.ForkJoinPool`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/ForkJoinPool.html), which is the execution engine behind parallel streams. However, that API is designed for compute-intensive tasks rather than tasks which involve I/O.)
...
- 
*Error handling with short-circuiting*— If one of the`findUser()`or`fetchOrder()`subtasks fails, by throwing an exception, then the other is cancelled, i.e.,interrupted](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/lang/Thread.html#interrupt()), if it has not yet completed.
...
The`StructuredTaskScope<T, R>`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.html)API, where`T`is the result type of tasks forked in the scope and`R`is the result type of the`join`method, can be summarized as:
...
In the`handle()`example, the zero-parameter`open()`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.html#open())factory method creates and opens a`StructuredTaskScope`that implements the default*completion policy*, which is to fail if any subtask fails. Other policies can be implemented, as we shall seebelow](#Joiners), by passing a suitable`Joiner`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.Joiner.html)to the one-parameter`open`method.
...
After joining, the scope's owner can process the results of the subtasks using the`Subtask`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.Subtask.html)objects returned from the`fork`methods. The`Subtask::get`method throws an exception if called before joining.
...
To allow for cancellation, subtasks must be coded so that they finish as soon as possible wheninterrupted](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/lang/Thread.html#interrupt()). Subtasks that do not respond to interrupts because, e.g., they block on methods that are not interruptible, may delay the closing of a scope indefinitely. The`close`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.html#close())method always waits for threads executing subtasks to finish, even if the scope is cancelled. Execution cannot continue beyond the`close`method until the interrupted threads finish.
...
Other policies can be selected by creating a`StructuredTaskScope`with a suitable`StructuredTaskScoped.Joiner`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.Joiner.html). A`Joiner`object handles subtask completion and produces the outcome for the`join()`method. Depending on the joiner, the`join()`method may return a result, a stream of elements, or some other object.

The`Joiner`interface declares factory methods to create joiners for some common cases. For example, the factory method`anySuccessfulResultOrThrow()`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.Joiner.html#anySuccessfulResultOrThrow())returns a new joiner that yields the result of any subtask that completes successfully:
...
As soon as one subtask succeeds then the scope is cancelled, cancelling the unfinished subtasks, and`join()`returns the result of the successful subtask. If all subtasks fail then`join()`throws a`FailedException`with the exception from one of the subtasks as thecause](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/lang/Throwable.html#getCause()). This pattern can be useful in, e.g., server applications that require a result from any one of a collection of redundant services.

The factory method`allSuccessfulOrThrow()`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.Joiner.html#allSuccessfulOrThrow())returns a new joiner that, when all subtasks complete successfully, yields a stream of the subtasks:
...
- 
`awaitAll()`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.Joiner.html#awaitAll()), which returns a new joiner that simply waits for all subtasks to complete, whether successfully or not;

- 
`awaitAllSuccessfulOrThrow()`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.Joiner.html#awaitAllSuccessfulOrThrow()), which returns a new joiner that waits for all subtasks to complete successfully; and

- 
`allUntil(Predicate<Subtask<? extends T>> isDone)`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.Joiner.html#allUntil(java.util.function.Predicate)), which returns a new joiner that, when all subtasks complete successfully or else a predicate on a completed subtask returns`true`, cancels the enclosing scope and yields a stream of all the subtasks.
...
How exceptions are handled will depend on usage. The`join()`method throws a`FailedException`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.FailedException.html)when the scope is considered to have failed. In the`handle()`example, if a subtask fails then a`FailedException`is thrown with the exception from the failed subtask as the cause. In some cases it may be useful to add a`catch`block to the`try`-with-resources statement in order to handle exceptions after the scope is closed:
...
Theearlier summary](#Using-StructuredTaskScope)of the`StructuredTaskScope`API showed two static`open`methods. Athird such method](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.html#open(java.util.concurrent.StructuredTaskScope.Joiner,java.util.function.Function))accepts a`Joiner`together with a function which can produce aconfiguration object](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.Configuration.html)in order to set the scope's name for monitoring and management purposes, to set the scope's timeout, and to set thethread factory](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/ThreadFactory.html)to be used by the scope's`fork`methods to create threads,
...
The`timeout`is specified as a`java.time.Duration`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/time/Duration.html). If the timeout expires before or while waiting in the`join()`method then the scope is cancelled, which cancels all incomplete subtasks, and`join()`throws a`TimeoutException`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/StructuredTaskScope.TimeoutException.html).
...
The`com.sun.management.HotSpotDiagnosticsMXBean`](https://docs.oracle.com/en/java/javase/25/docs/api/jdk.management/com/sun/management/HotSpotDiagnosticMXBean.html#dumpThreads(java.lang.String,com.sun.management.HotSpotDiagnosticMXBean.ThreadDumpFormat))API can also be used to generate such thread dumps, either directly or indirectly via the platform`MBeanServer`](https://docs.oracle.com/en/java/javase/25/docs/api/java.management/javax/management/MBeanServer.html)and a local or remote JMX tool.
...
When the`StructuredTaskScope`API was incubating, the`fork`methods returned a`Future`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/Future.html). This provided a sense of familiarity, by making these methods resemble the existing`ExecutorService::submit`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/ExecutorService.html#submit(java.lang.Runnable))method. However, the fact that`StructuredTaskScope`is intended to be used in a structured way, while`ExecutorService`is not, brought more confusion than clarity.

- 
The familiar use of`Future`involves calling its`get()`method, which blocks until a result is available. But in the context of`StructuredTaskScope`, using`Future`in this way is not only discouraged, but counterproductive. Structured`Future`objects should be queried only after the`join()`method returns, at which point they are known to be completed, and the method that should be used is not the familiar`get()`method but rather the recently introduced`resultNow()`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/Future.html#resultNow())method, which does not block.
