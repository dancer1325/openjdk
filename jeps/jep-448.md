...
| Effort | S | 
| Duration | S | 
| Relates to | JEP 438: Vector API (Fifth Incubator)](438) | 
...
| Issue | 8305868](https://bugs.openjdk.org/browse/JDK-8305868) | 
...
The Vector API was first proposed byJEP 338](https://openjdk.org/jeps/338)and integrated into JDK 16 as anincubating API](http://openjdk.org/jeps/11). Further rounds of incubation were proposed byJEP 414](https://openjdk.org/jeps/414)(integrated into JDK 17),JEP 417](https://openjdk.org/jeps/417)(JDK 18),JEP 426](https://openjdk.org/jeps/426)(JDK 19), andJEP 438](https://openjdk.org/jeps/438)(JDK 20).

This JEP proposes to re-incubate the API in JDK 21, with minor enhancements in the API relative to JDK 20. The implementation includes bug fixes and performance enhancements. We include the following notable changes:
...
- 
*Alignment with Project Valhalla*— The long-term goal of the Vector API is to leverageProject Valhalla](http://openjdk.org/projects/valhalla/)'s enhancements to the Java object model. Primarily this will mean changing the Vector API's currentvalue-based classes](https://docs.oracle.com/en/java/javase/16/docs/api/java.base/java/lang/doc-files/ValueBased.html)to be value classes so that programs can work with value objects, i.e., class instances that lack object identity. Accordingly, the Vector API will incubate over multiple releases until the necessary features of Project Valhalla become available aspreview features](https://openjdk.org/jeps/12). Once these Valhalla features are available we will adapt the Vector API and implementation to use them and then promote the Vector API itself to a preview feature. For further details, see the sections onrun-time compilation](#run-time-compilation)andfuture work](#future-work).
