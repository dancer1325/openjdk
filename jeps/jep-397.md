...
| Issue | 8246775](https://bugs.openjdk.org/browse/JDK-8246775) | 

## Summary

Enhance the Java programming language withsealed classes and interfaces](https://cr.openjdk.java.net/~briangoetz/amber/datum.html). Sealed classes and interfaces restrict which other classes or interfaces may extend or implement them. This is apreview language feature](http://openjdk.java.net/jeps/12)in JDK 16.

## History

Sealed Classes were proposed byJEP 360](https://openjdk.java.net/jeps/360)and delivered inJDK 15](https://openjdk.java.net/projects/jdk/15)as apreview feature](https://openjdk.java.net/jeps/12).
...
Every permitted subclass must use a modifier to describe how it propagates the sealing initiated by its superclass:
  ...
  - 
A permitted subclass may be declared`final`to prevent its part of the class hierarchy from being extended further. (Record classes (JEP 395](https://openjdk.java.net/jeps/395)) are implicitly declared`final`.)
...
Sealed classes work well with record classes (JEP 395](https://openjdk.java.net/jeps/395)). Record classes are implicitly`final`, so a sealed hierarchy of record classes is slightly more concise than the example above:
...
This JEP will extend the definition ofnarrowing reference conversion](https://docs.oracle.com/javase/specs/jls/se15/html/jls-5.html#jls-5.1.6.1)to navigate sealed hierarchies to determine at compile time which conversions are not possible.
...
In contrast, in the following code that uses a pattern matching`switch`expression, the compiler can confirm that every permitted subclass of`Shape`is covered, so no`default`clause (or other total pattern) is needed. The compiler will, moreover, issue an error message if any of the three cases are missing:
...
Sealed classes do not depend on record classes (JEP 395](https://openjdk.java.net/jeps/395)) or pattern matching (JEP 394](https://openjdk.java.net/jeps/394)), but they work well with both.