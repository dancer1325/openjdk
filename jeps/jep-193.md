...
| Effort | M | 
| Duration | L | 
| Relates to | JEP 266: More Concurrency Updates](266) | 
| Reviewed by | Dave Dice, Paul Sandoz | 
| Endorsed by | Brian Goetz | 
| Created | 2014/01/06 20:00 | 
| Updated | 2017/08/17 16:45 | 
| Issue | 8046183](https://bugs.openjdk.org/browse/JDK-8046183) | 
...
Variable handles are modelled by a single abstract class,`java.lang.invoke.VarHandle`, where each variable access mode is represented by asignature-polymorphic](http://docs.oracle.com/javase/8/docs/api/java/lang/invoke/MethodHandle.html#sigpoly)method.
...
Thus a`VarHandle`may be used in erased or reflective scenarios by a wrapping class, for example replacing the`Unsafe`usages within the`java.util.concurrent.Atomic*FieldUpdater/Atomic*Array`classes. (Although further work is required such that the updaters are granted access to the look up fields in the declaring class.)
...
- Allow signature-polymorphic methods to return types other than Object, indicating that the return type is not polymorphic (and would otherwise be declared via a cast at the call site). This makes it easier invoke write-based access methods that return void and invoke`compareAndSet`that returns a`boolean`value.
...
- An intrinsic for`Class.cast`, which has already been added (seeJDK-8054492](https://bugs.openjdk.java.net/browse/JDK-8054492)). Before this intrinsic was added a constant folded`Class.cast`would leave behind redundant checks that may cause unnecessary de-optimizations.
...
SeeJDK-8133348](https://bugs.openjdk.java.net/browse/JDK-8133348).
...
Introducing new forms of "value type" were considered that support volatile operations. However, this would be inconsistent with properties of other types, and would also require more effort for programmers to use. Reliance upon`java.util.concurrent.atomic``FieldUpdater`s was also considered, but their dynamic overhead and usage limitations make them unsuitable.
...
Generic types extending from`VarHandle`were considered in a previous version of this JEP but such an addition, with enhanced polymorphic signatures for generic types and special treatment of boxed type variables, was considered immature given a future Java release with value types and generics over primitives withJEP 218](http://openjdk.java.net/jeps/218), and improved arrays withArrays 2.0](http://cr.openjdk.java.net/~jrose/pres/201207-Arrays-2.pdf).
...
Stress tests will be developed using thejcstress](http://openjdk.java.net/projects/code-tools/jcstress/)harness.
...
This JEP does*not*depend onJEP 188: Java Memory Model Update](http://openjdk.java.net/jeps/188).