...
| Effort | XS | 
| Duration | XS | 
| Relates to | JEP 469: Vector API (Eighth Incubator)](469) | 
|  | JEP 508: Vector API (Tenth Incubator)](508) | 
| Reviewed by | Jatin Bhateja, Sandhya Viswanathan, Vladimir Ivanov | 
| Endorsed by | John Rose | 
| Created | 2024/09/24 19:20 | 
| Updated | 2025/04/10 17:49 | 
| Issue | 8340841](https://bugs.openjdk.org/browse/JDK-8340841) | 
...
We first proposed the Vector API inJEP 338](https://openjdk.org/jeps/338), integrated into JDK 16 as anincubating API](https://openjdk.org/jeps/11). We proposed further rounds of incubation inJEP 414](https://openjdk.org/jeps/414)(integrated into JDK 17),JEP 417](https://openjdk.org/jeps/417)(JDK 18),JEP 426](https://openjdk.org/jeps/426)(JDK 19),JEP 438](https://openjdk.org/jeps/438)(JDK 20),JEP 448](https://openjdk.org/jeps/448)(JDK 21),JEP 460](https://openjdk.org/jeps/460)(JDK 22), andJEP 469](https://openjdk.org/jeps/469)(JDK 23).

We propose here to re-incubate the API in JDK 24 with the following notable changes:

- 
A new variation of the`selectFrom`cross-lane operation accepts two input vectors, which serve as a lookup table. This complements the corresponding existing`rearrange`operation.

- 
The`selectFrom`and`rearrange`cross-lane operations now wrap indexes rather than check for out-of-bounds indexes. In combination with other implementation improvements, these operations are now significantly faster. As a result, the Vector API can express efficient SIMD algorithms that use table lookups.

- 
Transcendental and trigonometric lanewise operations on ARM and RISC-V are now implemented via intrinsics which call out to theSIMD Library for Evaluating Elementary Functions (SLEEF)](https://sleef.org).

- 
The new value-based class`Float16`](https://download.java.net/java/early_access/jdk24/docs/api/jdk.incubator.vector/jdk/incubator/vector/Float16.html)represents 16-bit floating-point numbers in theIEEE 754 binary16](https://en.wikipedia.org/wiki/Half-precision_floating-point_format)format. In the future we intend to auto-vectorize`Float16`operations on supporting hardware, enhance the Vector API to cover elements that are`Float16`values, and, eventually, migrate`Float16`to become a value class whenProject Valhalla](https://openjdk.org/projects/valhalla/)becomes available.

- 
The arithmetic integral lanewise operations now include:

  - Saturating unsigned addition and subtraction,
  - Saturating signed addition and subtraction, and
  - Unsigned maximum and minimum.

Equivalent scalar methods are declared in the new class`VectorMath`, and we specify the lanewise operations in terms of these new scalar methods.

The Vector API will incubate until necessary features ofProject Valhalla](https://openjdk.org/projects/valhalla/)become available aspreview features](https://openjdk.org/jeps/12). At that time, we will adapt the Vector API and its implementation to use them, and will promote the Vector API from incubation to preview.
...
HotSpot already supportsauto-vectorization](https://cr.openjdk.org/~vlivanov/talks/2017_Vectorization_in_HotSpot_JVM.pdf), which transforms scalar operations into superword operations which are then mapped to vector instructions. The set of transformable scalar operations is limited, and also fragile with respect to changes in code shape. Furthermore, only a subset of the available vector instructions might be utilized, limiting the performance of generated code.

Today, a developer who wishes to write scalar operations that are reliably transformed into superword operations needs to understand HotSpot's auto-vectorization algorithm and its limitations in order to achieve reliable and sustainable performance. In some cases it may not be possible to write scalar operations that are transformable. For example, HotSpot does not transform the simple scalar operations for calculating the hash code of an array (theArrays::hashCode](https://github.com/openjdk/jdk/blob/2a406f3ce5e200af9909ce051fdeed0cc059fea0/src/java.base/share/classes/java/util/Arrays.java#L4251)methods), nor can it auto-vectorize code to lexicographically compare two arrays (thus weadded an intrinsic for lexicographic comparison](https://bugs.openjdk.org/browse/JDK-8033148)).
...
The set of shapes supported correspond to vector sizes of 64, 128, 256, and 512 bits, as well as*max*bits. A 512-bit shape can pack`byte`s into 64 lanes or pack`int`s into 16 lanes, and a vector of such a shape can operate on 64`byte`s at a time or 16`int`s at a time. A*max*-bits shape supports the maximum vector size of the current architecture. This enables support for the ARM SVE platform, where platform implementations can support any fixed size ranging from 128 to 2048 bits, in increments of 128 bits.
...
The combination of element type and shape determines a vector's*species*, represented by`VectorSpecies<E>`](https://download.java.net/java/early_access/jdk24/docs/api/jdk.incubator.vector/jdk/incubator/vector/VectorSpecies.html).
...
To support control flow, some vector operations optionally accept masks represented by the public abstract class`VectorMask<E>`](https://download.java.net/java/early_access/jdk24/docs/api/jdk.incubator.vector/jdk/incubator/vector/VectorMask.html). Each element in a mask is a boolean value corresponding to a vector lane. A mask selects the lanes to which an operation is applied: It is applied if the mask element for the lane is true, and some alternative action is taken if the mask is false.
...
To support cross-lane permutation operations, some vector operations accept shuffles represented by the public abstract class`VectorShuffle<E>`](https://download.java.net/java/early_access/jdk24/docs/api/jdk.incubator.vector/jdk/incubator/vector/VectorShuffle.html). Each element in a shuffle is an`int`value corresponding to a lane index. A shuffle is a mapping of lane indexes, describing the movement of lane elements from a given vector to a result vector.
...
To avoid an explosion of C2 intrinsics we define generalized intrinsics corresponding to the various kinds of operations such as unary, binary, conversion, and so on, which take a parameter describing the specific operation to be performed. Approximately twenty-five new intrinsics support the intrinsification of the entire API.
...
In the future, ifProject Panama](https://openjdk.org/projects/panama/)expands its support of native calling conventions to support vector values then it may be possible for the Vector API implementation to load the SVML library from an external source. If there is no performance impact with this approach then it would no longer be necessary to include SVML in source form and build it into the JDK. Until then we deem the above approach acceptable, given the potential performance gains.

### Future work

- As mentioned above, we expect ultimately to declare vector classes asvalue classes](https://openjdk.org/jeps/401). For ongoing efforts to align the Vector API with Valhalla see thelworld+vector](https://github.com/openjdk/valhalla/tree/lworld+vector)branch of Project Valhalla's code repository. We expect, further, to leverage Project Valhalla’s generic specialization of value classes so that instances of`Vector<E>`are value objects, where`E`is a primitive class such as`int`instead of its boxed class`Integer`. Subtypes of`Vector<E>`for specific types, such as`IntVector`, might not be required once we have generic specialization over primitive classes.

- We may add support for vectors of IEEE floating-point binary16 values (float16 values). This also has a dependency on Project Valhalla, requiring that we represent a float16 value as a value object, with optimized layout in arrays and fields, and enhance the Vector API implementation to leverage vector hardware instructions on vectors of float16 values. For exploratory work see thevectorIntrinsics+fp16](https://github.com/openjdk/panama-vector/tree/vectorIntrinsics+fp16)branch of Project Panama's Vector API code repository.
