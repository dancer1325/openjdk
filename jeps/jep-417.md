...
| Reviewed by | John Rose, Vladimir Ivanov, Vladimir Kozlov | 
| Endorsed by | John Rose, Vladimir Kozlov | 
| Created | 2021/06/24 16:44 | 
| Updated | 2023/05/12 15:35 | 
| Issue | 8269306](https://bugs.openjdk.org/browse/JDK-8269306) | 
...
The Vector API was first proposed byJEP 338](https://openjdk.java.net/jeps/338)and integrated into Java 16 as anincubating API](http://openjdk.java.net/jeps/11). A second round of incubation was proposed byJEP 414](https://openjdk.java.net/jeps/414)and integrated into Java 17.
...
- 
Support theARM Scalar Vector Extension (SVE) platform](https://arxiv.org/pdf/1803.06185.pdf).
...
- 
We also anticipate enhancing the combinatorial units tests to assert that C2 generates vector hardware instructions. The unit tests currently assume, without verification, that vector hardware instructions will be generated via sufficient repeated execution. We will explore the use of C2'sIR Test Framework](https://github.com/openjdk/jdk/blob/master/test/hotspot/jtreg/compiler/lib/ir_framework/README.md)to assert, cross-platform, that vector nodes are present in the IR graph (for example, usingregex matching](https://bugs.openjdk.java.net/browse/JDK-8254129)). If this approach is problematic we may explore a rudimentary approach and use of the non-product`-XX:+TraceNewVectors`flag to print vector nodes.
