...
| Relates to | JEP 453: Structured Concurrency (Preview)](453) | 
| Reviewed by | Alex Buckley | 
| Endorsed by | Brian Goetz | 
| Created | 2022/10/28 12:41 | 
| Updated | 2023/06/08 16:58 | 
| Issue | 8296037](https://bugs.openjdk.org/browse/JDK-8296037) | 
...
Structured Concurrency was proposed byJEP 428](https://openjdk.org/jeps/428)and delivered inJDK 19](https://openjdk.org/projects/jdk/19/)as an incubating API. This JEP proposes to re-incubate the API, without changes, in JDK 20 to allow time for more feedback and to get more experience with this feature.
...
- 
Any of the forks in the scope, or the scope's owner, may call the scope's`shutdown()`](https://docs.oracle.com/en/java/javase/20/docs/api/jdk.incubator.concurrent/jdk/incubator/concurrent/StructuredTaskScope.html#shutdown())method to request cancellation of all remaining subtasks.

- 
The scope's owner joins the scope, i.e., all of its forks, as a unit. The owner can call the scope's`join()`](https://docs.oracle.com/en/java/javase/20/docs/api/jdk.incubator.concurrent/jdk/incubator/concurrent/StructuredTaskScope.html#join())method, which blocks until all forks have either completed (successfully or not) or been cancelled via`shutdown()`. Alternatively, the owner can call the scope's`joinUntil(java.time.Instant)`](https://docs.oracle.com/en/java/javase/20/docs/ap/jdk.incubator.concurrent/jdk/incubator/concurrent/StructuredTaskScope.html#joinUntil(java.time.Instant))method, which accepts a deadline.
...
The`com.sun.management.HotSpotDiagnosticsMXBean`](https://docs.oracle.com/en/java/javase/20/docs/api/jdk.management/com/sun/management/HotSpotDiagnosticMXBean.html#dumpThreads(java.lang.String,com.sun.management.HotSpotDiagnosticMXBean.ThreadDumpFormat))API can also be used to generate such thread dumps, either directly or indirectly via the platform`MBeanServer`](https://docs.oracle.com/en/java/javase/20/docs/api/java.management/javax/management/MBeanServer.html)and a local or remote JMX tool.
