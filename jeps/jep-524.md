...
| Effort | S | 
| Duration | S | 
| Relates to | JEP 470: PEM Encodings of Cryptographic Objects (Preview)](470) | 
| Reviewed by | Sean Mullan | 
| Endorsed by | Sean Mullan | 
| Created | 2025/06/25 21:32 | 
| Updated | 2025/12/16 17:57 | 
| Issue | 8360563](https://bugs.openjdk.org/browse/JDK-8360563) | 
...
The PEM API was proposed as a preview feature byJEP 470](https://openjdk.org/jeps/470)and delivered inJDK 25](https://openjdk.org/projects/jdk/25/). We here propose a second preview, to allow time for more feedback and to gain further experience with the feature.

Changes since the first preview:

- 
The`PEMRecord`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/PEMRecord.html)class is now named`PEM`](#The-PEM-class), and now includes a`decode()`method that returns the decoded Base64 content.

- 
The`encryptKey`methods of the`EncryptedPrivateKeyInfo`](#The-EncryptedPrivateKeyInfo-class)class are now named`encrypt`, and they now accept`DEREncodable`](#DER-encodable-cryptographic-objects)objects rather than`PrivateKey`objects, enabling the encryption of`KeyPair`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/KeyPair.html)and`PKCS8EncodedKeySpec`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/spec/PKCS8EncodedKeySpec.html)objects.

- 
The`EncryptedPrivateKeyInfo`](#The-EncryptedPrivateKeyInfo-class)class now includes`getKeyPair`methods that decrypt PKCS#8-encoded text containing a`PublicKey`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/PublicKey.html).

- 
The exceptions thrown by the`getKey`methods of the`EncryptedPrivateKeyInfo`](#The-EncryptedPrivateKeyInfo-class)class are now aligned with those thrown by the nearby`getKeySpec`methods.

- 
The`PEMEncoder`](#Encoding)and`PEMDecoder`](#Decoding)classes now support the encryption and decryption of`KeyPair`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/KeyPair.html)and`PKCS8EncodedKeySpec`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/spec/PKCS8EncodedKeySpec.html)objects.
...
- 
The`DEREncodable`](https://cr.openjdk.org/~ascarpino/pem26/api/java.base/java/security/DEREncodable.html)interface is implemented by Java Platform API classes representing cryptographic objects with binary-encodable key or certificate material.

- 
The`PEMEncoder`](https://cr.openjdk.org/~ascarpino/pem26/api/java.base/java/security/PEMEncoder.html)and`PEMDecoder`](https://cr.openjdk.org/~ascarpino/pem26/api/java.base/java/security/PEMDecoder.html)classes are for encoding to and decoding from the PEM format. Instances of these classes are immutable and reusable, i.e., they do not retain information from the previously encoded or decoded cryptographic object.

- 
The`PEM`](https://cr.openjdk.org/~ascarpino/pem26/api/java.base/java/security/PEM.html)class, which implements`DEREncodable`, is for encoding and decoding PEM text representing cryptographic objects for which no Java Platform API exists.

### This is apreview API](https://openjdk.org/jeps/12), disabled by default

To use this API in JDK 26, you must enable preview APIs:
...
We thus introduce a new interface,`DEREncodable`](https://cr.openjdk.org/~ascarpino/pem26/api/java.base/java/security/DEREncodable.html), to identify the cryptographic APIs that provide such conversions and whose instances can therefore be encoded to, and decoded from, the PEM format. This empty interface is sealed; its permitted classes and interfaces are`AsymmetricKey`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/AsymmetricKey.html),`X509Certificate`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/cert/X509Certificate.html),`X509CRL`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/cert/X509CRL.html),`KeyPair`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/KeyPair.html),`EncryptedPrivateKeyInfo`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/javax/crypto/EncryptedPrivateKeyInfo.html),`PKCS8EncodedKeySpec`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/spec/PKCS8EncodedKeySpec.html),`X509EncodedKeySpec`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/spec/X509EncodedKeySpec.html), and`PEM`](https://cr.openjdk.org/~ascarpino/pem26/api/java.base/java/security/PEM.html):
...
The`PEMEncoder`](https://cr.openjdk.org/~ascarpino/pem26/api/java.base/java/security/PEMEncoder.html)class declares methods for encoding`DEREncodable`objects into PEM text:
...
There are two methods for encoding. One method returns PEM text in a byte array containing characters encoded in theISO-8859-1](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/nio/charset/StandardCharsets.html#ISO_8859_1)charset; for example, to encode a private key:
...
With these methods, if the class is incorrect then a`ClassCastException`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/lang/ClassCastException.html)is thrown.
...
The existing`EncryptedPrivateKeyInfo`](https://cr.openjdk.org/~ascarpino/pem26/api/java.base/javax/crypto/EncryptedPrivateKeyInfo.html)class represents an encrypted private key. To make it easier to use with the`PEMEncoder`and`PEMDecoder`classes, we have added seven methods to it:
...
The new`getKey`methods decrypt the private key in an`EncryptedPrivateKeyInfo`instance. These methods accept a password and, optionally, a cryptographic provider, and return a`PrivateKey`. They can be used when a`PEMDecoder`returns an`EncryptedPrivateKeyInfo`:
...
The default password-based encryption (PBE) algorithm used when encrypting a`DEREncodable`with either`PEMEncoder`or`EncryptedPrivateKeyInfo`is defined in thedefault security properties file](https://docs.oracle.com/en/java/javase/25/security/security-properties-file.html). The`jdk.epkcs8.defaultAlgorithm`security property defines the default algorithm to be "PBEWithHmacSHA256AndAES_128". The default algorithm might change in the future, but this will not affect PEM text created today since the data encoded in that text contains the algorithm name and all other parameters necessary for decryption.

## Alternatives

A PEM API is a bridge between Base64 and cryptographic objects. We rejected many other potential designs because they did not fit well with the existing cryptographic APIs. While some of the alternatives might have been adequate, we chose the proposed API for its similarity to the`HexFormat`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/HexFormat.html)API and the nested`Encoder`and`Decoder`classes of the`Base64`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/Base64.html)API. We wanted to have immutability, thread safety, and distinct paths through the API for encoding and decoding.

Some of the alternatives we considered include:

- 
*Extend the`EncodedKeySpec`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/spec/EncodedKeySpec.html)API*— This API encapsulates binary-encoded key data for`KeyFactory`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/security/KeyFactory.html)instances and other cryptographic classes. A new`PEMEncodedKeySpec`subclass could type-identify encapsulated PEM text while providing encoding and decoding operations between PEM text and the appropriate private or public key`EncodedKeySpec`.
