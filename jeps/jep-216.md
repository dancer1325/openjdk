...
| Reviewed by | Alex Buckley, Brian Goetz, Jonathan Gibbons, Maurizio Cimadamore | 
| Endorsed by | Brian Goetz | 
| Created | 2014/08/26 13:05 | 
| Updated | 2016/07/12 20:59 | 
| Issue | 8056066](https://bugs.openjdk.org/browse/JDK-8056066) | 
...
In some cases`javac`will accept source code with a certain order of imports and reject the same source code with just the imports reordered (*e.g.*,JDK-7177813](https://bugs.openjdk.java.net/browse/JDK-7177813)). That is wrong and confusing.
...
Member resolution, which includes:
  ...
  - (**1a**) If`T`is toplevel,`import`s in the source file defining`T`are processed and imported members added in`T`'s scope
