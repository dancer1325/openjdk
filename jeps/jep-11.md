...
| Issue | 8169768](https://bugs.openjdk.org/browse/JDK-8169768) | 
...
- EnableJDK Release Projects](http://openjdk.java.net/bylaws#jdk-release-projects)to distribute a limited set of APIs and tools that not final and complete, and which would benefit from developer or user feedback. This will reduce the chance of costly mistakes in the Java SE Platform and the JDK.
...
- 
The JEP that defines the feature should make it clear that incubation is planned, and that the result of completing the JEP will be an incubator module which exports the API/contains the tool.

- 
An incubating feature, along with any evolutionary API changes, that is later promoted to the Java SE Platform, or some other stable mode of existence in the JDK, should have a new JEP to propose and document that change. For the new JEP, it is permitted, but not required, to deliver the promoted API or tool in its own module; the API or tool may be integrated into an existing standard module or a non-incubator JDK module.
...
- 
Incubator modules must not export APIs in the`java.`or`javax.`namespaces governed by the JCP. This distinguishes incubator modules from*standard modules*](http://openjdk.java.net/jeps/200)such as`java.base`.
...
- 
Incubator modules must not export*critical internal APIs*](http://openjdk.java.net/jeps/260). Incubator modules have no relationship to the`jdk.unsupported`module.
...
Early-access (EA) binaries of a JDK Release Project or another Project will continue to be offered so that developers can try new features and offer feedback before theGenerally Available](http://openjdk.java.net/projects/jdk8/milestones#General_Availability)(GA) release. However, the number of EA downloads has historically been much smaller than GA downloads, because most developers desire the higher level of stability and quality provided by GA releases. As such, EA releases garner only limited attention and generate only limited feedback, yet incubating features are aimed at a wide range of developers and thus seek broad attention and generous feedback. This makes the EA release of a feature an inadequate alternative to the GA release of an incubating feature.
