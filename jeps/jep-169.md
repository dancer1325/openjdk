...
| Updated | 2021/12/09 21:35 | 
| Issue | 8046159](https://bugs.openjdk.org/browse/JDK-8046159) | 
...
- Efficiently support temporary "scratch versions" of immutable Valhalla objects which can be mutated freely (in a single thread), before being promoted back to their shareable, freely copyable, identity-free form.
...
Additional design notes are keptin the MLVM repository](http://hg.openjdk.java.net/mlvm/mlvm/hotspot/raw-file/tip/value-obj-8001111.txt).

## Alternatives

Programmers currently must use what Rich Hickey callsplace-oriented programming](http://www.infoq.com/presentations/Value-Values)in mutable arrays or buffer objects, when working with composite values. They could simply continue this.
...
We could add newexplicit tuple types](http://blogs.oracle.com/jrose/entry/value_types_in_the_vm)to the JVM, providing a more explicit third option between classes and primitives.
...
(For more discussion of mutability during initialization of read-only objects, seelarval objects](http://blogs.oracle.com/jrose/entry/larval_objects_in_the_vm).)
