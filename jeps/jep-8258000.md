...
| Issue | 8258000](https://bugs.openjdk.org/browse/JDK-8258000) | 

`InstanceKlass`today serves triple duty:

- It records the contents of a loaded classfile plus resolution states.
- It represents a "live type" (a subtype of`Klass`) in the JVM bookkeeping.
...
- 
`ClassFile`(or`ClassFileInfo`,`ClassFileStructuure`, etc.) should be a loaded classfile, with an associated`ConstantPool`holding constants and their resolution states. Much of today's current contents of`InstanceKlass`should be renamed into this new class.

- 
`InstanceKlass`should become an abstract API with one implementation called`ClassFileKlass`, which contains a pointer to a`ClassFile`. These types should be "hollowed out", and only retain methods which seem directly applicable to a JVM bookkeeping over types. That includes methods shared with`Klass`,`arrayKlass`, etc.

- 
The object header can contain a direct`InstanceKlass`pointer, just as today. It can be encoded or decoded in the case of compression (or other tricks in the future).
...
- `InstanceKlass`is abstract (we wanted to do this in the first place)
- `ClassFileKlass`<:`InstanceKlass`(today's code, renamed)
- parametric paraphernalia added to`ClassInfo`
- `SpeciesKlass`<:`InstanceKlass`points to`ClassInfo`and parameter binding stuff
