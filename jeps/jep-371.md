...
- 
Deprecate thenon-standard API](https://blogs.oracle.com/jrose/anonymous-classes-in-the-vm)`sun.misc.Unsafe::defineAnonymousClass`, with the intent to deprecate it for removal in a future release.
...
Many language implementations built on the JVM rely upon dynamic class generation for flexibility and efficiency. For example, in the case of the Java language,`javac`does not translate a lambda expression into a dedicated`class`file at compile time but, rather, emits bytecode thatdynamically generates and instantiates a class](https://docs.oracle.com/en/java/javase/14/docs/api/java.base/java/lang/invoke/LambdaMetafactory.html)to yield an object corresponding to the lambda expression when needed. Similarly, runtimes for non-Java languages often implement the higher-order features of those languages by usingdynamic proxies](https://docs.oracle.com/en/java/javase/14/docs/api/java.base/java/lang/reflect/Proxy.html), which also generate classes dynamically.
...
- 
Final fields declared in a hidden class are not modifiable.`Field::set`and other setter methods on a final field of a hidden class will throw`IllegalAccessException`regardless of the field's`accessible`flag](https://docs.oracle.com/en/java/javase/14/docs/api/java.base/java/lang/reflect/AccessibleObject.html#setAccessible(boolean)).

- 
The`Class`object is not modifiable byinstrumentation agents](https://docs.oracle.com/en/java/javase/14/docs/api/java.instrument/java/lang/instrument/Instrumentation.html#isModifiableClass(java.lang.Class)), and cannot be redefined or retransformed by JVM TI agents. We will, however, extend JVM TI and JDI to support hidden classes, such as testing whether a class is hidden, including hidden classes in any list of "loaded" classes, and sending JVM TI events when hidden classes are created.
...
JEP 181 (Nest-Based Access Control)](http://openjdk.java.net/jeps/181)introduced nest-based access control contexts, in which all classes and interfaces in a nest share private access among the nestmates.