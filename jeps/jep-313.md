...
| Issue | 8182758](https://bugs.openjdk.org/browse/JDK-8182758) | 
...
The tool has been superseded by superior functionality in javac, added in JDK 8 (JDK-7150368](https://bugs.openjdk.java.net/browse/JDK-7150368)). This functionality provides the ability to write native header files at the time that Java source code is compiled, thereby eliminating the need for a separate tool.
...
There are no direct JDK dependences on the`javah`tool. Externally, there are some derivatives of`javah`, such as the Antjavah task](https://ant.apache.org/manual/Tasks/javah.html), but just as users of the`javah`command are recommended to use`javac -h`, so too are users of these dependences are recommended to use instead the corresponding support provided for`javac`.