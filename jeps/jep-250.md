...
The string table is compressed and then stored in the archive at dump time. The compression technique for the string table is the same as for the shared symbol table (seeJDK-8059510](https://bugs.openjdk.java.net/browse/JDK-8059510)). The regular narrow oop encoding and decoding is used to access the shared`String`objects from the compressed-string table.
...
The serviceability agent needs to be updated to add support for the shared-string table (seeJDK-8079830](https://bugs.openjdk.java.net/browse/JDK-8079830)).

With the change proposed byJDK-8054307](https://bugs.openjdk.java.net/browse/JDK-8054307), the underlying`char`array will be changed to be a`byte`array. The code that copies interned strings to the string space and perform deduplication will need to reflect that if and when JDK-8054307 is integrated. The impact should be minimal.