...
| Effort | XL | 
| Relates to | JEP 436: Virtual Threads (Second Preview)](436) | 
| Reviewed by | Alex Buckley, Brian Goetz, Chris Hegarty | 
| Created | 2021/11/15 16:43 | 
| Updated | 2023/06/07 09:43 | 
| Issue | 8277131](https://bugs.openjdk.org/browse/JDK-8277131) | 

## Summary

Introduce*virtual threads*to the Java Platform. Virtual threads are lightweight threads that dramatically reduce the effort of writing, maintaining, and observing high-throughput concurrent applications. This is apreview API](https://openjdk.java.net/jeps/12).
...
- 
It is not a goal to offer a new data parallelism construct in either the Java language or the Java libraries. TheStream API](https://docs.oracle.com/en/java/javase/18/docs/api/java.base/java/util/stream/package-summary.html)remains the preferred way to process large data sets in parallel.
...
We update the`java.lang.Thread`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html)API as follows:

- 
`Thread.Builder`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.Builder.html),`Thread.ofVirtual()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#ofVirtual()), and`Thread.ofPlatform()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#ofPlatform())are new APIs to create virtual and platform threads. For example,
...
- 
`Thread.startVirtualThread(Runnable)`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#startVirtualThread(java.lang.Runnable))is a convenient way to create and then start a virtual thread.

- 
A`Thread.Builder`can create either a thread or a`ThreadFactory`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/util/concurrent/ThreadFactory.html), which can then create multiple threads with identical properties.

- 
`Thread.isVirtual()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#isVirtual())tests whether a thread is a virtual thread.

- 
New overloads of`Thread.join`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#join(java.time.Duration))and`Thread.sleep`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#sleep(java.time.Duration))accept wait and sleep times as instances of`java.time.Duration`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/time/Duration.html).

- 
The new final method`Thread.threadId()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#threadId())returns a thread's identifier. The existing non-final method`Thread.getId()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#getId())is now deprecated.

- 
`Thread.getAllStackTraces()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#getAllStackTraces())now returns a map of all platform threads rather than all threads.
...
To allow for interruption and cancellation, the blocking I/O methods defined by`java.net.Socket`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/net/Socket.html),`ServerSocket`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/net/ServerSocket.html), and`DatagramSocket`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/net/DatagramSocket.html)are now specified to beinterruptible](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#interrupt())when invoked in a virtual thread: Interrupting a virtual thread blocked on a socket will unpark the thread and close the socket. Blocking I/O operations on these types of sockets when obtained from an`InterruptibleChannel`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/nio/channels/InterruptibleChannel.html)have always been interruptible, so this change aligns the behavior of these APIs when created with their constructors with their behavior when obtained from a channel.
...
The updates toJVM TI](https://docs.oracle.com/en/java/javase/19/docs/specs/jvmti.html)are:
...
The updates toJDWP](https://docs.oracle.com/en/java/javase/19/docs/specs/jdwp/jdwp-protocol.html)are:
...
The updates toJDI](https://docs.oracle.com/en/java/javase/19/docs/api/jdk.jdi/module-summary.html)are:

- 
A new method in`com.sun.jdi.ThreadReference`](https://docs.oracle.com/en/java/javase/19/docs/api/jdk.jdi/com/sun/jdi/ThreadReference.html#isVirtual())tests if a thread is a virtual thread.

- 
New methods in`com.sun.jdi.request.ThreadStartRequest`](https://docs.oracle.com/en/java/javase/19/docs/api/jdk.jdi/com/sun/jdi/request/ThreadStartRequest.html#addPlatformThreadsOnlyFilter())and`com.sun.jdi.request.ThreadDeathRequest`](https://docs.oracle.com/en/java/javase/19/docs/api/jdk.jdi/com/sun/jdi/request/ThreadDeathRequest.html#addPlatformThreadsOnlyFilter())limit the events generated for the request to platform threads.
...
A new method in`com.sun.management.HotSpotDiagnosticsMXBean`](https://docs.oracle.com/en/java/javase/19/docs/api/jdk.management/com/sun/management/HotSpotDiagnosticMXBean.html#dumpThreads(java.lang.String,com.sun.management.HotSpotDiagnosticMXBean.ThreadDumpFormat))generates the new-style thread dump describedabove](#observability). This method can also be invoked indirectly via the platform`MBeanServer`](https://docs.oracle.com/en/java/javase/19/docs/api/ava.management/javax/management/MBeanServer.html)from a local or remote JMX tool.

### `java.lang.ThreadGroup`

`java.lang.ThreadGroup`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/ThreadGroup.html)is a legacy API for grouping threads that is rarely used in modern applications and unsuitable for grouping virtual threads. We deprecate and degrade it now, and expect to introduce a new thread-organizing construct in the future as part ofstructured concurrency](https://openjdk.java.net/jeps/8277129).

As background, the`ThreadGroup`API dates from Java 1.0. It was originally intended to provide job-control operations such as stopping all threads in a group. Modern code is more likely to use the thread pool APIs of the`java.util.concurrent`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/util/concurrent/package-summary.html)package, introduced in Java 5.`ThreadGroup`supported the isolation of applets in early Java releases, but the Java security architecture evolved significantly in Java 1.2 and thread groups no longer played a significant role.`ThreadGroup`was also intended to be useful for diagnostic purposes, but that role was superseded by the monitoring and management features introduced in Java 5, including the`java.lang.management`](https://docs.oracle.com/en/java/javase/19/docs/api/java.management/java/lang/management/package-summary.html)API.
...
- 
The ability to explicitly a destroy a thread group is removed: The terminally deprecated`destroy()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/ThreadGroup.html#destroy())method does nothing.

- 
The notion of daemon thread groups is removed: The daemon status set and retrieved by the terminally deprecated`setDaemon(boolean)`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/ThreadGroup.html#setDaemon(boolean))and`isDaemon()`](https://docs.oracle.com/en/java/javase/19/docs/api/ava.base/java/lang/ThreadGroup.html#isDaemon())methods is ignored.
...
- 
The terminally deprecated`suspend()`](https://docs.oracle.com/en/java/javase/19/docs/api/ava.base/java/lang/ThreadGroup.html#suspend()),`resume()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/ThreadGroup.html#resume()), and`stop()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/ThreadGroup.html#stop())methods always throw an exception.
...
- 
`java.lang.ThreadGroup`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/ThreadGroup.html)no longer allows a thread group to bedestroyed](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/ThreadGroup.html#destroy()), no longer supports the notion ofdaemon thread groups](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/ThreadGroup.html#setDaemon(boolean)), and its`suspend()`](https://docs.oracle.com/en/java/javase/19/docs/api/ava.base/java/lang/ThreadGroup.html#suspend()),`resume()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/ThreadGroup.html#resume()), and`stop()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/ThreadGroup.html#stop())methods always throw exceptions.
...
- 
If code in an existing source file extends`Thread`and a method in the subclass conflicts with any of the new`Thread`methods then the file will not compile without change.
...
- 
The`Thread.setPriority(int)`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#setPriority(int))method has no effect on virtual threads, which always have a priority of`Thread.NORM_PRIORITY`.

- 
The`Thread.setDaemon(boolean)`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#setDaemon(boolean))method has no effect on virtual threads, which are always daemon threads.

- 
TheThread.`stop()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#stop()),`suspend()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#suspend()), and`resume()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#resume())methods throw an`UnsupportedOperationException`when invoked on a virtual thread.

- 
The`Thread`API supports the creation of threads that do not support thread-local variables.`ThreadLocal.set(T)`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/ThreadLocal.html#set(T))and`Thread.setContextClassLoader(ClassLoader)`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#setContextClassLoader(java.lang.ClassLoader))throw an`UnsupportedOperationException`when invoked in the context of a thread that does not support thread locals.

- 
`Thread.getAllStackTraces()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#getAllStackTraces())now returns a map of all platform threads rather than a map of all threads.
...
- 
Virtual threads are not active members of a`ThreadGroup`. Invoking`Thread.getThreadGroup()`](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Thread.html#getThreadGroup())on a virtual thread returns a dummy`"VirtualThreads"`group that is empty.
...
- 
The`java.lang.management.ThreadMXBean`](https://docs.oracle.com/en/java/javase/19/docs/api/java.management/java/lang/management/ThreadMXBean.html)API supports the monitoring and management of platform threads, but not virtual threads.
