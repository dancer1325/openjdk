...
| Effort | S | 
| Duration | S | 
| Depends | JEP 140: Limited doPrivileged](140) | 
| Reviewed by | Alan Bateman, Chris Hegarty, Jeffrey Nisewanger | 
| Endorsed by | Brian Goetz | 
| Created | 2013/03/26 20:00 | 
| Updated | 2017/09/04 15:30 | 
| Issue | 8046173](https://bugs.openjdk.org/browse/JDK-8046173) | 

## Summary

Implement the W3CCross Origin Resource Sharing (CORS) protocol](http://www.w3.org/TR/cors/)in order to align Java's HTTP stack with HTML5 and current web browsers.

## Motivation

Java code running under a security manager is currently restricted by thesame origin policy](http://en.wikipedia.org/wiki/Same_origin_policy): Network connections can only be made to the code's origin unless explicit permissions are granted.

The World-Wide Web is moving towards a less-restrictive policy based on an open standard calledCross Origin Resource Sharing (CORS)](http://www.w3.org/TR/cors/). CORS defines a protocol by which a web client can interact with a server to authorize specific cross-origin requests.
...
- 
Whether credentials (*i.e.*, cookies or HTTP authentication) are to be used in this potentially cross-origin request; and

- 
Whether a pre-flight request should be forced, even if a (pre-flight) result has already been cached.
...
TheCORS protocol definition](http://www.w3.org/TR/cors/)is stable but not yet final, so this feature might not initially be implemented as a public API in the`java.net`package. If not then it will be implemented as a JDK-specific API or system property, which could evolve as needed along with the protocol.
...
TheCORS protocol definition](http://www.w3.org/TR/cors/)is a W3C "Candidate Recommendation" as of January 2013. It was intended to become a "Proposed Recommendation" by the beginning of March, but that did not happen. If the CORS protocol definition changes then this feature will have to be revised accordingly.

## Dependences

This feature depends onJEP 140: Limited doPrivileged](https://bugs.openjdk.org/browse/JDK-8046130).

## Impact

- 
Compatibility: New behavior will be enabled through a new API or a system property.
...
- 
TCK: One new method added to the existing`HttpURLConnection`class, plus some minor changes to existing`java.security`classes.